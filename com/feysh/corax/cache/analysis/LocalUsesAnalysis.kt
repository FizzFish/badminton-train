...
public data class LocalUsesAnalysis(graph: DirectedGraph<Unit>, locals: Set<Local>, mode: FlowAnalysisMode) : AnalysisKey(
      LocalUsesAnalysisFactory.INSTANCE.getKey()
   ) {
   ...
   public fun copy(graph: DirectedGraph<Unit> = this.graph, locals: Set<Local> = this.locals, mode: FlowAnalysisMode = this.mode): LocalUsesAnalysis {
   ...
   public override operator fun equals(other: Any?): Boolean {
      if (this === other) {
         return true;
      } else if (other !is LocalUsesAnalysis) {
         return false;
      } else {
         val var2: LocalUsesAnalysis = other as LocalUsesAnalysis;
         if (!(this.graph == (other as LocalUsesAnalysis).graph)) {
            return false;
