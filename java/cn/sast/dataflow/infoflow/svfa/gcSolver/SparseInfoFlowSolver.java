...
public class SparseInfoFlowSolver extends InfoflowSolver {
    ...
    public void propagate(@NotNull Abstraction sourceVal, @NotNull Unit target, @NotNull Abstraction targetVal, @NotNull Unit relatedCallSite, boolean isUnbalancedReturn) {
        List<Unit> success;
        List list;
        boolean z;
        boolean z2;
        ...
        Intrinsics.checkNotNullParameter(relatedCallSite, "relatedCallSite");
        if (targetVal.getAccessPath().getPlainValue() == null || Intrinsics.areEqual(targetVal.getAccessPath(), this.zeroValue.getAccessPath())) {
            super.propagate((FastSolverLinkedNode) sourceVal, target, (FastSolverLinkedNode) targetVal, relatedCallSite, isUnbalancedReturn);
            return;
        }
        ...
        for (Pair pair : propagates) {
            ...
            if (isBackward()) {
                ...
                if (dominator.getUnit() != null && (dominator.getUnit() instanceof IfStmt)) {
                    super.propagate((FastSolverLinkedNode) sourceVal, useUnit, fastSolverLinkedNode.deriveNewAbstractionWithDominator(dominator.getUnit()), relatedCallSite, isUnbalancedReturn);
                }
            }
            super.propagate((FastSolverLinkedNode) sourceVal, useUnit, fastSolverLinkedNode, relatedCallSite, isUnbalancedReturn);
        }
    }
