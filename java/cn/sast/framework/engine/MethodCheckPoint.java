...
/* loaded from: MethodCheckPoint.class */
public final class MethodCheckPoint extends CheckPoint implements IMethodCheckPoint, SootInfoCache {
    ...
    public MethodCheckPoint(@NotNull SootMethod sootMethod, @NotNull SootInfoCache info) {
        ...
        this.info = info;
        SootClass declaringClass = getSootMethod().getDeclaringClass();
    ...
    public VisibilityAnnotationTag getVisibilityAnnotationTag() {
        return getSootMethod().getTag("VisibilityAnnotationTag");
    }

    public void eachUnit(@NotNull Function1<? super IUnitCheckPoint, Unit> function1) {
        Intrinsics.checkNotNullParameter(function1, "block");
        if (!getSootMethod().hasActiveBody()) {
            return;
        }
        Body b = getSootMethod().getActiveBody();
        Iterator it = b.getUnits().iterator();
        Intrinsics.checkNotNullExpressionValue(it, "iterator(...)");
        while (it.hasNext()) {
            soot.Unit unit = (soot.Unit) it.next();
            SootInfoCache sootInfoCache = this.info;
            Intrinsics.checkNotNull(unit);
            function1.invoke(new UnitCheckPoint(sootInfoCache, unit, getSootMethod()));
        }
    }

    @NotNull
    public Region getRegion() {
        Region regionInvoke = Region.Companion.invoke(this, getSootMethod());
