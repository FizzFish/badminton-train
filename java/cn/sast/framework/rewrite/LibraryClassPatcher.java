...
/* loaded from: LibraryClassPatcher.class */
public class LibraryClassPatcher {

    /* loaded from: LibraryClassPatcher$IMessageObtainCodeInjector.class */
    ...
    private void patchMessageObtainImplementation() {
        ...
        if (smObtain1 != null && (!smObtain1.hasActiveBody() || systemClassHandler.isStubImplementation(smObtain1.getActiveBody()))) {
            generateMessageObtainMethod(smObtain1, new IMessageObtainCodeInjector() { // from class: cn.sast.framework.rewrite.LibraryClassPatcher.1
                @Override // cn.sast.framework.rewrite.LibraryClassPatcher.IMessageObtainCodeInjector
                public void injectCode(Body body, Local messageLocal) {
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldWhat.makeRef()), body.getParameterLocal(1)));
                }
            });
        }
        ...
        if (smObtain2 != null && (!smObtain2.hasActiveBody() || systemClassHandler.isStubImplementation(smObtain2.getActiveBody()))) {
            generateMessageObtainMethod(smObtain2, new IMessageObtainCodeInjector() { // from class: cn.sast.framework.rewrite.LibraryClassPatcher.2
                @Override // cn.sast.framework.rewrite.LibraryClassPatcher.IMessageObtainCodeInjector
                public void injectCode(Body body, Local messageLocal) {
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldWhat.makeRef()), body.getParameterLocal(1)));
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldArg1.makeRef()), body.getParameterLocal(2)));
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldArg2.makeRef()), body.getParameterLocal(3)));
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldObj.makeRef()), body.getParameterLocal(4)));
                }
            });
        }
        ...
        if (smObtain3 != null && (!smObtain3.hasActiveBody() || systemClassHandler.isStubImplementation(smObtain3.getActiveBody()))) {
            generateMessageObtainMethod(smObtain3, new IMessageObtainCodeInjector() { // from class: cn.sast.framework.rewrite.LibraryClassPatcher.3
                @Override // cn.sast.framework.rewrite.LibraryClassPatcher.IMessageObtainCodeInjector
                public void injectCode(Body body, Local messageLocal) {
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldWhat.makeRef()), body.getParameterLocal(1)));
                    body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldArg1.makeRef()), body.getParameterLocal(2)));
        ...
        if (smObtain4 != null) {
            if (!smObtain4.hasActiveBody() || systemClassHandler.isStubImplementation(smObtain4.getActiveBody())) {
                generateMessageObtainMethod(smObtain4, new IMessageObtainCodeInjector() { // from class: cn.sast.framework.rewrite.LibraryClassPatcher.4
                    @Override // cn.sast.framework.rewrite.LibraryClassPatcher.IMessageObtainCodeInjector
                    public void injectCode(Body body, Local messageLocal) {
                        body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldWhat.makeRef()), body.getParameterLocal(1)));
                        body.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(messageLocal, fldObj.makeRef()), body.getParameterLocal(2)));
                    }
                });
            }
        }
    ...
    private void patchActivityImplementation() {
        ...
        if (smRun != null) {
            ...
            JimpleBody jimpleBodyNewBody = Jimple.v().newBody(smRun);
            smRun.setActiveBody(jimpleBodyNewBody);
            Local thisLocal = Jimple.v().newLocal("this", sc.getType());
            jimpleBodyNewBody.getLocals().add(thisLocal);
            jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newThisRef(sc.getType())));
            ...
            jimpleBodyNewBody.getLocals().add(targetLocal);
            jimpleBodyNewBody.getUnits().add(Jimple.v().newAssignStmt(targetLocal, Jimple.v().newStaticFieldRef(appStaticFieldRef)));
            jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnStmt(targetLocal));
        }
    }
    ...
    private void patchThreadImplementation() {
        ...
        if (smRun != null) {
            ...
            if (smStart != null) {
                if (smStart.hasActiveBody() && !systemClassHandler.isStubImplementation(smStart.getActiveBody())) {
                    return;
                }
                ...
                if (smCons != null) {
                    ...
                    SootField fldTarget = Scene.v().makeSootField("target" + fieldIdx, runnable.getType());
    ...
    private void patchThreadRunMethod(SootMethod smRun, SootClass runnable, SootField fldTarget) {
        ...
        jimpleBodyNewBody.getLocals().add(targetLocal);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newAssignStmt(targetLocal, Jimple.v().newInstanceFieldRef(thisLocal, fldTarget.makeRef())));
        ReturnVoidStmt returnVoidStmtNewReturnVoidStmt = Jimple.v().newReturnVoidStmt();
        jimpleBodyNewBody.getUnits().add(Jimple.v().newIfStmt(Jimple.v().newEqExpr(targetLocal, NullConstant.v()), returnVoidStmtNewReturnVoidStmt));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newInvokeStmt(Jimple.v().newInterfaceInvokeExpr(targetLocal, runnable.getMethod("void run()").makeRef())));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newInvokeStmt(Jimple.v().newInterfaceInvokeExpr(thisLocal, runnable.getMethod("void run()").makeRef())));
        jimpleBodyNewBody.getUnits().add(returnVoidStmtNewReturnVoidStmt);
    }
    ...
    private void patchThreadConstructor(SootMethod smCons, SootClass runnable, SootField fldTarget) {
        ...
        JimpleBody jimpleBodyNewBody = Jimple.v().newBody(smCons);
        smCons.setActiveBody(jimpleBodyNewBody);
        Local thisLocal = Jimple.v().newLocal("this", sc.getType());
        jimpleBodyNewBody.getLocals().add(thisLocal);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newThisRef(sc.getType())));
        Local param0Local = Jimple.v().newLocal("p0", runnable.getType());
        jimpleBodyNewBody.getLocals().add(param0Local);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(param0Local, Jimple.v().newParameterRef(runnable.getType(), 0)));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newAssignStmt(Jimple.v().newInstanceFieldRef(thisLocal, fldTarget.makeRef()), param0Local));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnVoidStmt());
    }
    ...
    private Body patchHandlerDispatchBody(SootMethod method) {
        ...
        JimpleBody jimpleBodyNewBody = Jimple.v().newBody(method);
        method.setActiveBody(jimpleBodyNewBody);
        Local thisLocal = Jimple.v().newLocal("this", sc.getType());
        jimpleBodyNewBody.getLocals().add(thisLocal);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newThisRef(sc.getType())));
        ...
        for (int i = 0; i < method.getParameterCount(); i++) {
            ...
            jimpleBodyNewBody.getLocals().add(paramLocal);
            jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(paramLocal, Jimple.v().newParameterRef(method.getParameterType(i), i)));
            if (i == 0) {
                firstParam = paramLocal;
            }
        }
        jimpleBodyNewBody.getUnits().add(Jimple.v().newInvokeStmt(Jimple.v().newVirtualInvokeExpr(thisLocal, Scene.v().makeMethodRef(sc, "handleMessage", Collections.singletonList(method.getParameterType(0)), VoidType.v(), false), firstParam)));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnVoidStmt());
        return jimpleBodyNewBody;
    }
    ...
    private Body patchHandlerPostBody(SootMethod method, SootClass runnable) {
        ...
        jimpleBodyNewBody.getUnits().add(Jimple.v().newInvokeStmt(Jimple.v().newInterfaceInvokeExpr(firstParam, Scene.v().makeMethodRef(runnable, "run", Collections.emptyList(), VoidType.v(), false))));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnStmt(IntConstant.v(1)));
        return jimpleBodyNewBody;
    }
    ...
    private void patchTimerScheduleMethod(SootMethod method) {
        ...
        if (runMethod != null) {
            jimpleBodyNewBody.getUnits().add(Jimple.v().newInvokeStmt(Jimple.v().newVirtualInvokeExpr(firstParam, runMethod.makeRef())));
        }
        jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnVoidStmt());
    }
    ...
    private void patchActivityGetFragmentManager() {
        ...
        JimpleBody jimpleBodyNewBody = Jimple.v().newBody(smGetFM);
        smGetFM.setActiveBody(jimpleBodyNewBody);
        Local thisLocal = Jimple.v().newLocal("this", sc.getType());
        jimpleBodyNewBody.getLocals().add(thisLocal);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newThisRef(sc.getType())));
        ...
        jimpleBodyNewBody.getLocals().add(retLocal);
        jimpleBodyNewBody.getUnits().add(Jimple.v().newAssignStmt(retLocal, Jimple.v().newNewExpr(scFragmentTransaction.getType())));
        jimpleBodyNewBody.getUnits().add(Jimple.v().newReturnStmt(retLocal));
    }
}
